...
/* MOBILE: tap on parent link toggles dropdown */
document.querySelectorAll(".has-dropdown > a").forEach((link) => {
  link.addEventListener("click", function (e) {
    if (window.innerWidth <= 820) {
      const dd = this.closest(".has-dropdown");
      const wasOpen = dd.classList.contains("mob-open");
      // close all others
      document
...
const sections = [
  ...
  {
    id: "features",
    link: document.querySelector('.nav-links a[href="#features"]'),
  },
  { id: "analytics", link: null },
  {
    id: "how-it-works",
    link: document.querySelector('.nav-links a[href="#how-it-works"]'),
  },
];
const navH = () => document.querySelector(".navbar").offsetHeight;
...
/* HEATMAP */
const heatLevels = [
  0, 0, 1, 2, 3, 1, 0, 1, 2, 3, 4, 2, 0, 0, 3, 4, 2, 1, 0, 3, 1, 2, 1, 3, 4, 1,
  0, 2,
];
...
function renderCal() {
  ...
  const cnt = Object.keys(sessions).filter((k) =>
    k.startsWith(`${calYear}-${calMonth}-`),
  ).length;
